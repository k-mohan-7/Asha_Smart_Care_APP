...
public class VaccinationListActivity extends AppCompatActivity implements VaccinationAdapter.OnVaccinationClickListener {
    ...
    private void loadData() {
        ...
        vaccinationList.addAll(dbHelper.getVaccinationsByPatientId(patientId));
        updateUI();
        showLoading(false);
    ...
    private void updateUI() {
        ...
        for (Vaccination v : vaccinationList) {
            if ("Given".equals(v.getStatus())) given++;
        }
        tvSubtitle.setText(given + "/" + vaccinationList.size() + " vaccinations completed");
    ...
    public void onVaccinationClick(Vaccination vaccination) {
        Intent intent = new Intent(this, AddVaccinationActivity.class);
        intent.putExtra("patient_id", patientId);
        intent.putExtra("vaccination_id", vaccination.getId());
        startActivity(intent);
    }
